  ...
  <section class="bg-white rounded-xl shadow p-4">
    <div class="flex flex-wrap gap-4 items-end">
      <div class="flex-1 min-w-[200px]">
        <label for="managerFilter" class="block text-xs font-semibold text-gray-500 mb-1">Manager</label>
        <select id="managerFilter" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0053e2]">
...
[managerFilter, shiftFilter, searchBox, overdueOnly].forEach(el => {{
  el.addEventListener("input", renderTable);
  el.addEventListener("change", renderTable);
}});

document.getElementById("clearFilters").addEventListener("click", () => {{
  managerFilter.value = ""; shiftFilter.value = ""; searchBox.value = ""; overdueOnly.checked = false;
